...
public class QccResolver implements PageProcessor {
    ...
    public void process(Page page) {
        ...
        QccEnt qccEnt = new QccEnt();


        Html html =  page.getHtml();
        Document document = html.getDocument();

        Elements ents = document.getElementsByClass("frtrt");
        if(ents.size()==0)return;
        String imgUrl=document.getElementsByClass("frtrt").first().getElementsByTag("img").attr("src");
        String href=document.getElementsByClass("title").first().attr("href");
        String name=document.getElementsByClass("title").first().child(0).text().replace("</em>","").replace("<em>","");
        String url=request.getUrl();
        //解析标题
        qccEnt.keyNo =href.replace("https://www.qcc.com/firm/","").replace(".html","");
        qccEnt.imageUrl =imgUrl;
        qccEnt.name=name;
        qccEnt.url="";
        qccEnt.setImgId(Base64Util.encodeURLSafeString(imgUrl));
        page.putField("qccEnt", qccEnt);
        try {
